...
vector<User> readAccountHistory()
{
	...
	if(userFile.is_open())
	{
		// Variables:
		string line;
		string uname, password, lastplay, prem;
		...
		while(getline(userFile, line))
		{
			...
			winstreak = stoi(line.substr(47, 12));

			// Get last play from line
			lastplay = removeWhiteSpaces(line.substr(59, 18));

			// Determine whether premium user
			premium = removeWhiteSpaces(line.substr(77, 8)).find("X") != string::npos;

			User user(uname, password, wins, losses, winpct,
				winstreak, lastplay, premium);
...
void displayUserMsg(User user)
{
	...
	printMessage("3. Upgrade to premium", false, false);
	printMessage("4. Logout", false, true);
	cout << endl;
}


void displayPremiumMsg(User user)
{
	cout << endl;
	printMessage("Hello " + user.getUsername(), true, false);
	printMessage("1. Play a game", true, false);
	printMessage("2. Check your history", false, false);
	printMessage("3. Clear History", false, false);
	printMessage("4. Check rank", false, false);
	printMessage("5. Logout", false, true);
	cout << endl;
}
...
void displayAdminMessage()
{
	...
	printMessage("4. Erase account history file", false, false);
	printMessage("5. Create a new user", false, false);
	printMessage("6. Logout", false, true);
	cout << endl;
}
...
void create_new_user(User user)
{
	...
	cout << "Enter password: ";
	getline(cin, password);
	if (user.createNewUser(username, password))
...
int main()
{
	...
	while (in != 'q')
	{
		user_vector = readAccountHistory();
		// ---------------------
		// Display ADMIN menu
		// ---------------------
		if (currentUser.is_admin())
		{
			...
			case '4':
				currentUser.resetFile();
				break;

			case '5':
				create_new_user(currentUser);
				break;
		...
		// ---------------------
		// Display default LOGGED IN menu
		// ---------------------
		else if(currentUser.getUsername() != "TEMP" && !currentUser.isPremium())
		{
		...
		// ---------------------
		// Display premium LOGGED IN menu
		// ---------------------
		else if (currentUser.getUsername() != "TEMP" && currentUser.isPremium())
		{
			displayPremiumMsg(currentUser);
			cout << "Please select a number to continue, enter 'q' to quit: ";
			getline(cin, input);
			in = input[0];
			if (input.length() > 1)
			...
			case '4':
				currentUser.checkRank(currentUser, user_vector);
				break;
